...
target_alias=gaspard-*-elf
target_vendor=COPYING
target_os=COPYING.LIB-COPYING3-COPYING3.LIB-ChangeLog-MAINTAINERS-Makefile.def-Makefile.in-Makefile.tpl-README-README-maintainer-mode-ar-lib-bfd-binutils-build-compile-confdefs.h-config-config-ml.in-config.guess-config.log-config.rpath-config.sub-configure-configure.ac-cpu-depcomp-elfcpp-etc-gas-gold-gprof-include-install-sh-intl-ld-libctf-libiberty-libtool.m4-ltgcc.m4-ltmain.sh-ltoptions.m4-ltsugar.m4-ltversion.m4-lt~obsolete.m4-makefile.vms-missing-mkdep-mkinstalldirs-move-if-change-opcodes-setup.com-sha256.sum-src-release.sh-symlink-tree-test-driver-texinfo-ylwrap-zlib-elf
target=gaspard-*-elf

program_transform_name = s&^&gaspard-*-elf-&
...
TOPLEVEL_CONFIGURE_ARGUMENTS=./configure --target='gaspard-*-elf'

tooldir = ${exec_prefix}/gaspard-*-elf
build_tooldir = ${exec_prefix}/gaspard-*-elf
...
BUILD_CONFIGARGS =  --cache-file=./config.cache  --program-transform-name='s&^&gaspard-*-elf-&' --disable-option-checking --with-build-subdir="$(BUILD_SUBDIR)"
...
SUBDIRS =  intl libiberty opcodes bfd zlib libctf binutils gas ld gprof etc
TARGET_CONFIGDIRS = 
# This is set by the configure script to the arguments to use when configuring
# directories built for the host system.
HOST_CONFIGARGS =  --cache-file=./config.cache  --with-gnu-as --with-gnu-ld  --program-transform-name='s&^&gaspard-*-elf-&' --disable-option-checking
...
TARGET_SUBDIR = gaspard-*-elf
# This is set by the configure script to the arguments to use when configuring
# directories built for the target.
TARGET_CONFIGARGS = --cache-file=./config.cache --enable-multilib --with-cross-host=x86_64-pc-linux-gnu   --program-transform-name='s&^&gaspard-*-elf-&' --disable-option-checking --with-target-subdir="$(TARGET_SUBDIR)"
# This is the list of variables to export in the environment when
...
AS = as
AR = ar --plugin /usr/local/libexec/gcc/x86_64-pc-linux-gnu/11.1.0/liblto_plugin.so
AR_FLAGS = rc
CC = gcc
CXX = g++
DLLTOOL = dlltool
...
NM = nm
OBJDUMP = objdump
RANLIB = ranlib --plugin /usr/local/libexec/gcc/x86_64-pc-linux-gnu/11.1.0/liblto_plugin.so
READELF = readelf
STRIP = strip
WINDRES = windres
WINDMC = windmc
...
CC_FOR_TARGET=$(STAGE_CC_WRAPPER) gaspard-*-elf-cc

# If GCC_FOR_TARGET is not overriden on the command line, then this
# variable is passed down to the gcc Makefile, where it is used to
# build libgcc2.a.  We define it here so that it can itself be
# overridden on the command line.
GCC_FOR_TARGET=$(STAGE_CC_WRAPPER) gaspard-*-elf-gcc
CXX_FOR_TARGET=$(STAGE_CC_WRAPPER) gaspard-*-elf-c++
RAW_CXX_FOR_TARGET=$(STAGE_CC_WRAPPER) gaspard-*-elf-c++
GFORTRAN_FOR_TARGET=$(STAGE_CC_WRAPPER) gaspard-*-elf-gfortran
GOC_FOR_TARGET=$(STAGE_CC_WRAPPER) gaspard-*-elf-gccgo
DLLTOOL_FOR_TARGET=$$r/$(HOST_SUBDIR)/binutils/dlltool
LD_FOR_TARGET=$$r/$(HOST_SUBDIR)/ld/ld-new

LIPO_FOR_TARGET=gaspard-*-elf-lipo
